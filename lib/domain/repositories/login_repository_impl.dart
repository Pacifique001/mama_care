...
class LoginRepositoryImpl implements LoginRepository {
  final DatabaseHelper _databaseHelper; // Keep for potential local checks if needed
  ...
  Future<UserCredential> login(String email, String password) async {
    ...
    try {
      ...
      if (userCredential.user != null) {
        ...
        if (token != null) {
          await _updateUserFCMTokenInFirestore(userCredential.user!.uid, token);
           ...
           _logger.w("Could not get FCM token after login for user ${userCredential.user!.uid}");
        }
        ...
        // Avoid direct DB updates here to prevent race conditions/redundancy.
         ...
         throw AuthException("Login failed: User data unavailable after successful authentication.");
      }
      ...
      return userCredential;
       ...
       _logger.w("Repository: Firebase login failed for $email. Code: ${e.code}");
      ...
      throw AuthException(_parseFirebaseErrorMsg(e.code, "Login failed"));
       ...
       _logger.e("Repository: Unexpected login error for $email", error: e, stackTrace: stackTrace);
      throw AuthException("An unexpected error occurred during login."); // Generic error
    }
  }
  ...
  Future<UserCredential> googleLogin() async {
    ...
    try {
      ...
      if (googleUser == null) {
        ...
        throw AuthException("Google Sign-In cancelled.", isCancelled: true); // Specific exception
      }
      ...
      // Update FCM token in Firestore
       if (userCredential.user != null) {
         ...
         // Local DB sync handled by AuthViewModel listener
          ...
          _logger.e("Firebase Google Sign-In succeeded but UserCredential.user is null!");
          // Sign out from google in case of partial failure
           await _googleSignIn.signOut().catchError((err) => _logger.e("Error signing out Google on failure", error: err));
          throw AuthException("Google Sign-In failed: User data unavailable after successful authentication.");
       }

        _logger.i("Repository: Firebase Google Sign-In successful for ${googleUser.email}");
      return userCredential;
    } on FirebaseAuthException catch (e) {
        _logger.w("Repository: Firebase Google Sign-In failed. Code: ${e.code}");
        await _googleSignIn.signOut().catchError((err) => _logger.e("Error signing out Google on failure", error: err));
        throw AuthException(_parseFirebaseErrorMsg(e.code, "Google Sign-In failed"));
    } catch (e, stackTrace) {
       _logger.e("Repository: Unexpected Google Sign-In error", error: e, stackTrace: stackTrace);
       await _googleSignIn.signOut().catchError((err) => _logger.e("Error signing out Google on failure", error: err));
       if (e is AuthException && e.isCancelled) rethrow; // Don't wrap cancellation error
  ...
  Future<void> _updateUserFCMTokenInFirestore(String uid, String token) async {
    try {
      ...
      await _firestore.collection('users').doc(uid).set({
       ...
       _logger.d("Repository: Firestore FCM token updated successfully for user $uid");
    } catch (e, stackTrace) {
       _logger.e('Repository: Error updating FCM token in Firestore for user $uid', error: e, stackTrace: stackTrace);
  ...
  Future<UserCredential> signUp({
    ...
    try {
      final userCredential = await _firebaseAuth.createUserWithEmailAndPassword(
       ...
       _logger.d("Repository: Firebase user created: ${userCredential.user?.uid}");

      // Update Firebase profile immediately after creation
      if (userCredential.user != null) {
         User firebaseUser = userCredential.user!;
         try {
             _logger.d("Repository: Updating Firebase profile for ${firebaseUser.uid}");
             ...
             // Phone number requires verification - handle separately if needed
              ...
              _logger.w("Repository: Failed to update Firebase profile after signup", error: e, stackTrace: stackTrace);
             // Non-fatal, continue
         }
          ...
          await _firebaseMessaging.subscribeToTopic('general').catchError((e) =>
         ...
         throw AuthException("Signup failed: User data unavailable after successful creation.");
      }
      ...
      return userCredential;
       ...
       _logger.w("Repository: Firebase signup failed for $email. Code: ${e.code}");
      throw AuthException(_parseFirebaseErrorMsg(e.code, "Sign-up failed"));
    } catch (e, stackTrace) {
       _logger.e("Repository: Unexpected sign-up error for $email", error: e, stackTrace: stackTrace);
  ...
  Future<void> logout() async {
    ...
    try {
      ...
      if (user != null) {
        ...
        await _firebaseMessaging.unsubscribeFromTopic('general').catchError((e) =>
          _logger.e("Error unsubscribing from topic 'general' on logout", error: e)
        );
  ...
  Future<void> sendEmailOTP(String email) async {
     _logger.e("Repository: sendEmailOTP called on client-side. This MUST be handled by the backend for security.");
     throw UnimplementedError("OTP sending must be performed by a secure backend service.");
     // --- REMOVED SMTP and local OTP storage logic ---
  }

  @override
  Future<bool> verifyEmailOTP(String email, String otp) async {
    _logger.e("Repository: verifyEmailOTP called on client-side. This MUST be handled by the backend for security.");
    throw UnimplementedError("OTP verification must be performed by a secure backend service.");
  ...
  Future<void> saveUserToLocalDatabase(Map<String, dynamic> userData) async {
     _logger.w("Repository: saveUserToLocalDatabase called. This is likely redundant if AuthViewModel handles local user sync.");
  ...
  String _parseFirebaseErrorMsg(String errorCode, String defaultPrefix) {
     switch (errorCode) {
      ...
      default: return '$defaultPrefix: An unknown error occurred ($errorCode).';
    }
  }
